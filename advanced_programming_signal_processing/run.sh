...
# 			if [ $x = 0 ]
# 			then
# 				# printf '%s ' $r | xargs -d' ' -I{} -P$(nproc) ./matching $name "rotated/{}/${btpl}" {} 1.5 cwp 
# 				for rotation in $r; do
# 					./matching $name "rotated/${rotation}/${btpl}" $rotation 1.5 cwp 
# 				done
# 				x=1
# 			else
# 				# printf '%s ' $r | xargs -d' ' -I{} -P$(nproc) ./matching $name "rotated/{}/${btpl}" {} 1.5 wp 
# 				for rotation in $r; do
# 					./matching $name "rotated/${rotation}/${btpl}" $rotation 1.5 wp 
# 				done
# 			fi
# 		}
# 		export -f run_single_image6	# -f: unsupported
# 		;;
# 	*)
...
# 			done
# 		}
# 		export -f run_single_image	# -f: unsupported
# 		;;
# esac
...
case $level in
	level1)
		printf '%s ' $images | xargs -d' ' -I{} -P$(nproc) sh run_single_image.sh {} "$templates" 0 0.5 pg
		;;
	level2)
		printf '%s ' $images | xargs -d' ' -I{} -P$(nproc) sh run_single_image.sh {} "$templates" 0 1.5 p
		;;
	level3)
		for image in $images;
		do
		...
		printf '%s ' $processed_images | xargs -d' ' -I{} -P$(nproc) sh run_single_image.sh {} "$templates" 0 1.5 p
		;;
	level4)
		printf '%s ' $images | xargs -d' ' -I{} -P$(nproc) sh run_single_image.sh {} "$templates" 0 0.5 pb 
		;;
	level[57])
		# 特徴マッチング
	...
	level6)
		...
		# 	# echo $bname:
		# 	for template in $templates; do
		# 		sh -c "$run_single_image6 $template" &	# TODO: separate the function into a file
		# 	done
		# 	# echo ""
		# done
		;;
